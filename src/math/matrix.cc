  ...
  int m, int n, int ia, int ja)
{
  ...
  int ione = 1;
  int gictxt;
  Cblacs_get( 0, 0, &gictxt );  //DCY //CS //need this to get proj matrix col sum correct
  //ewd:  Cblacs_get returns wrong context handle for multiple k-points
  //gictxt = ictxt_;   //DCY //CS //need this to get proj matrix col sum correct
    ...
    if ( t.active() )
    {
      ...
      for ( int jj = 0; jj < blocksize; jj++ )
      {
        for ( int ii = 0; ii < m_; ii++ )
        {
          ...
          //os << "(" << ii << "," << jj+jb*maxbs << ")="
          //   << t.val[ii+t.mloc()*jj] << endl;
          os << ii << " " << jj+jb*maxbs << " " << t.val[ii+t.mloc()*jj] << endl;
        }
        //ewd DEBUG
        cout << endl;
      }
